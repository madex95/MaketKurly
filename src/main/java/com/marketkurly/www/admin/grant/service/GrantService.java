...
public class GrantService {
	...
	public void insertExGrant(String code, List<String> abled) {
		List<String> exGrant = grantMapper.getExGrantAbledNameList();
		ExGrantAbledList grantAbled = new ExGrantAbledList();
		grantAbled.setGrant_code(code);
		if (abled != null) {
			for (String i : abled) {
				exGrant.remove(i);
				grantAbled.setExGrant_code(i);
				if (grantMapper.getExGrantAbled(grantAbled) == 0)
					grantMapper.insertExGrant(grantAbled);
			}
		}
		for (String i : exGrant) {
			grantAbled.setExGrant_code(i);
			grantMapper.deleteExGrant(grantAbled);
		}
	}
